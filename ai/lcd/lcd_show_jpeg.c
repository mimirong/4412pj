...
int fb_pixel_rgba(void *fbmem, int width, int height,
               int x, int y,
...
int show_jpeg(char *path)
{
	...
	while (cinfo.output_scanline < cinfo.output_height) {
       ...
       } else if (fb_depth == 32) { // 添加一个32位的处理，我的fb是设置的32bit的色深
			unsigned short color;
			for (x = 0; x < cinfo.output_width; x++) {
				...
				//	, buffer[x*3+2]
				//	);
				fb_pixel_rgba(fbmem, fb_width, fb_height, x, y,
					buffer[x*3+0]
					, buffer[x*3+1]
					, buffer[x*3+2]
					, 1
